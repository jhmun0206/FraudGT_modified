...
# EllipticPPPyG_TxWallet은 조건부로 import (파일이 없을 수 있음)
try:
    from fraudGT.datasets.ellipticpp_tx_wallet_pyg import EllipticPPPyG_TxWallet
except ImportError:
    EllipticPPPyG_TxWallet = None
...
def load_dataset_master(format, name, dataset_dir):
    # custom hook for ellipticpp
    if name.lower() == 'ellipticpp' and str(format).lower() in ['pyg', 'pytorch_geometric', 'tg']:
        # Tx+Wallet 이종 그래프는 별도 name으로 처리되므로 여기서는 제거
        
        # 이종 그래프 모드 확인
        hetero_mode = getattr(cfg.dataset, 'hetero_mode', 'none') if hasattr(cfg, 'dataset') else 'none'
        is_hetero = getattr(cfg.dataset, 'hetero', False) if hasattr(cfg, 'dataset') else False
        
        # 방법 A: 클러스터링 기반 이종 그래프
        if hetero_mode == 'v2_cluster':
            from fraudGT.datasets.ellipticpp_hetero_cluster import EllipticPPPyG_HeteroCluster
            logging.info("[INFO] Using EllipticPPPyG_HeteroCluster (clustering-based heterogeneous graph)")
            return EllipticPPPyG_HeteroCluster(dataset_dir)
        
        # 기존 v2 버전 (하위 호환성)
        elif is_hetero:
            hetero_version = getattr(cfg.dataset, 'hetero_version', None) if hasattr(cfg, 'dataset') else None
            if hetero_version == 'v2':
                from fraudGT.datasets.ellipticpp_hetero_pyg_v2 import EllipticPPPyG_HeteroV2
                logging.info("[INFO] Using EllipticPPPyG_HeteroV2 (real heterogeneous graph)")
                return EllipticPPPyG_HeteroV2(dataset_dir)
            else:
                # 기존 버전 (단순 이종화)
    ...
    # custom hook for ellipticpp_tx_wallet
    if name.lower() == 'ellipticpp_tx_wallet' and str(format).lower() in ['pyg', 'pytorch_geometric', 'tg']:
        if EllipticPPPyG_TxWallet is None:
            raise ImportError("EllipticPPPyG_TxWallet를 import할 수 없습니다. fraudGT.datasets.ellipticpp_tx_wallet_pyg 파일이 존재하는지 확인하세요.")
        logging.info("[INFO] Using EllipticPPPyG_TxWallet (tx+wallet heterogeneous graph)")
