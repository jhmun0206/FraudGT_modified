...
def eval_epoch(logger, loader, model, split='val'):
    ...
    # Collect predictions and labels for test split
    if split == 'test':
        all_preds = []
        all_labels = []
    ...
    # Save predictions and labels for test split
    if split == 'test' and len(all_preds) > 0:
        pred = torch.cat(all_preds)
        true = torch.cat(all_labels)
        log_dir = cfg.run_dir
        os.makedirs(log_dir, exist_ok=True)
        np.save(os.path.join(log_dir, "test_predictions.npy"), pred.cpu().numpy())
        np.save(os.path.join(log_dir, "test_labels.npy"), true.cpu().numpy())
