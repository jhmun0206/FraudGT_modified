...
def agg_dict_list(dict_list):
    ...
    # Get all keys from all dictionaries (union of all keys)
    all_keys = set()
    for dict in dict_list:
        all_keys.update(dict.keys())
    # Aggregate all keys (except 'epoch')
    for key in all_keys:
        if key != 'epoch':
            # Only include keys that exist in all dictionaries (or handle missing keys)
            values = []
            for dict in dict_list:
                if key in dict:
                    values.append(dict[key])
            if len(values) == len(dict_list):  # Key exists in all dicts
                value = np.array(values)
                dict_agg[key] = np.mean(value).round(cfg.round)
                dict_agg['{}_std'.format(key)] = np.std(value).round(cfg.round)
            elif len(values) > 0:  # Key exists in some dicts (use available values)
                value = np.array(values)
                dict_agg[key] = np.mean(value).round(cfg.round)
                dict_agg['{}_std'.format(key)] = np.std(value).round(cfg.round)
    return dict_agg
