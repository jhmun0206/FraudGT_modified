...
def json_to_dict_list(fname):
    dict_list = []
    epoch_dict = {}  # Store the most complete dict for each epoch
    with open(fname) as f:
        ...
        for line in lines:
            ...
            epoch = dict['epoch']
            # Keep the dict with the most keys (most complete)
            if epoch not in epoch_dict:
                epoch_dict[epoch] = dict
            else:
                # Replace if current dict has more keys
                if len(dict.keys()) > len(epoch_dict[epoch].keys()):
                    epoch_dict[epoch] = dict
    # Convert to list sorted by epoch
    dict_list = [epoch_dict[epoch] for epoch in sorted(epoch_dict.keys())]
    return dict_list
