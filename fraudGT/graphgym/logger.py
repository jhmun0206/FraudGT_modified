...
class Logger(object):
    ...
    def classification_binary(self):
        ...
        # AUC for binary can use raw scores; guard against single-class batch
        try:
        ...
        return {
            ...
            # micro-f1 is identical to standard f1 for binary, but we log it explicitly for consistency
            'micro-f1': round(f1_score(true, pred_int, average='micro'), cfg.round),
    ...
    def classification_multi(self):
        ...
        prec_macro, rec_macro, f1_macro, _ = precision_recall_fscore_support(
            true, pred_int, average='macro', zero_division=0
        )
        # Micro metrics
        prec_micro, rec_micro, f1_micro, _ = precision_recall_fscore_support(
            true, pred_int, average='micro', zero_division=0
        )

        # Multi-class AUC (One-vs-Rest, macro-averaged). May fail if a class is missing in a batch.
        try:
            auc_macro = roc_auc_score(true, pred_score, multi_class='ovr', average='macro')
        except ValueError:
            auc_macro = 0.0

        return {
            'accuracy': round(accuracy_score(true, pred_int), cfg.round),
            'precision': round(prec_macro, cfg.round),
            'recall': round(rec_macro, cfg.round),
            'f1': round(f1_macro, cfg.round),
            'micro-f1': round(f1_micro, cfg.round),
            'auc': round(auc_macro, cfg.round),
        }
    ...
    def write_epoch(self, cur_epoch):
        ...
        # Prevent custom stats (e.g., batch-averaged precision/recall/auc) from overriding
        # the epoch-level task metrics computed above.
        # Keep only custom keys that are not already present in task_stats.
        custom_stats_all = self.custom()
        custom_stats = {k: v for k, v in custom_stats_all.items() if k not in task_stats}
